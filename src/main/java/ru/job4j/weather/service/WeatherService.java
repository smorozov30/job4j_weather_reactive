...
public class WeatherService {
...

    {
        ...
        weathers.put(2, new Weather(2, "SPb", 30));
    ...
    public Mono<Weather> findHottest() {
        return Mono.justOrEmpty(
                weathers.values().stream().max(Comparator.comparingInt(Weather::getTemperature)).get()
        );
    }

    public Flux<Weather> findAllHotterThan(Integer t) {
        return Flux.fromIterable(
                weathers.values().stream().filter(weather -> weather.getTemperature() > t).collect(Collectors.toList())
        );
    }
}
